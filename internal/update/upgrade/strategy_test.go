...
func TestRunStrategy_BetaGentleAISelfUpgradeUsesGoInstallMain(t *testing.T) {
	...
	wantArgs := []string{"install", "github.com/franwerner/gentle-ai/v3/cmd/gentle-ai@main"}
	if len(gotArgs) != len(wantArgs) || gotArgs[0] != wantArgs[0] || gotArgs[1] != wantArgs[1] {
		t.Fatalf("exec args = %v, want %v", gotArgs, wantArgs)
	}
	for _, want := range []string{
		"GONOSUMDB=github.com/franwerner/gentle-ai/v3",
		"GOPRIVATE=github.com/franwerner/gentle-ai/v3",
		"GONOPROXY=github.com/franwerner/gentle-ai/v3",
	} {
...
func TestGoProxyBypassEnvPreservesExistingPatterns(t *testing.T) {
	module := "github.com/franwerner/gentle-ai/v3"
	...
	for _, want := range []string{
		"PATH=/usr/bin",
		"GONOSUMDB=github.com/franwerner/gentle-ai/v3,example.com/private",
		"GOPRIVATE=github.com/franwerner/gentle-ai/v3,github.com/acme/*",
		"GONOPROXY=github.com/franwerner/gentle-ai/v3",
	} {
...
func TestEffectiveMethodGentleAIOnWindowsUsesFailClosedBinaryPolicy(t *testing.T) {
	...
	t.Run("Go availability upgrades through a pinned go install", func(t *testing.T) {
		tool := update.ToolInfo{Name: "gentle-ai", InstallMethod: update.InstallBinary, GoImportPath: "github.com/franwerner/gentle-ai/v3/cmd/gentle-ai"}
		profile := system.PlatformProfile{OS: "windows", PackageManager: "winget", GoAvailable: true}
